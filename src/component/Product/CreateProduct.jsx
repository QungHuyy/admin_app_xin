...
function CreateProduct(props) {
    ...
    const addProduct = async () => {
        try {
            ...
            // Gửi request đến search-by-ai.onrender.com để phân tích đặc trưng hình ảnh
            ...
            if (response.statuscode === 200 || response.status === 200 || response.msg === "Bạn đã thêm thành công") {
                setName('');
                setPrice('');
                setDescription('');
                setInventoryS('0');
                ...
                setValidationMsg({ api: "Bạn đã thêm thành công" });
            } else {
                // Nếu có lỗi, hiển thị thông báo lỗi từ server
                setValidationMsg({ api: response.msg || "Có lỗi xảy ra khi thêm sản phẩm" });
            }
        } catch (error) {
            console.error("Error adding product:", error);
            setValidationMsg({ api: "Có lỗi xảy ra khi thêm sản phẩm" });
        }
    }
